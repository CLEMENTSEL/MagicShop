            ...
            <section className="container mx-auto p-10 md:py-12 px-0 md:p-8 md:px-0">
              <div className="p-5 md:p-0 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-10 items-start">
                {product && Array.isArray(product) && product.map((r: any, i: number) => (
                  <div className="p-5 bg-white bg-opacity-90 text-center transform duration-500 hover:-translate-y-2 rounded-2xl text-black min-h-full">
                    <Link href={`/home/admin/${r.id}`}>
                      <h1 className="text-2xl my-5">ID: {r.id}</h1>
                      <Image src={r.image} width={1000} height={1000} alt="Produit" className='cursor-pointer' />
                    ...
                    <Formik
                      ...
                      onSubmit={() => {
                        axios.delete(`http://127.0.0.1:8000/products/${r.id}`, {
                          withCredentials: true,
                        })
                          .then(response => {
                            if (response.status === 200) {
