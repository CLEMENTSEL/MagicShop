...
export default function ModifyProduct({ params }: {
  ...
  if (access_token === undefined) {
    ...
    useEffect(() => {
      axios.get((`http://127.0.0.1:8000/products/${params.productID}`), {
        ...
        <nav className="bg-white border-gray-200 dark:bg-gray-900">
          <div className="flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div className="flex items-center md:order-2 space-x-3 md:space-x-4 rtl:space-x-reverse">
              <Link href={`/home/admin/createProduct`}>
          ...
          <section className="container mx-auto px-0 md:px-0 w-2/4">
            <div className="md:p-0 flex gap-10 items-center">
              {product && Array.isArray(product) && product.map((r: any, i: number) => (
                <div className="p-5 bg-white bg-opacity-90 text-center transform duration-500 hover:-translate-y-2 rounded-2xl text-black min-h-full max-w-1/2" id="cardProduct" key={r.id}>
                  ...
                  <p className="text-sm mb-5 h-full min-h-20">{r.description}</p>
            ...
            <div className="static">
              <Formik
                ...
                onSubmit={async (values) => {
                  console.log(values);
                  axios.put(`http://127.0.0.1:8000/products/${params.productID}`, {
                    data: {
                      image: values.image,
              ...
              <Formik
                ...
                onSubmit={ () => {
                  axios.delete(`http://127.0.0.1:8000/products/${params.productID}`, {
                    withCredentials: true,
                  })
                    .then(response => {
                      if (response.status === 200) {
