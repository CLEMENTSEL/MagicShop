...
function getOrders(props: any) {
  let [products, getProduct] = useState('');
  useEffect(() => {
    axios.get((`http://127.0.0.1:8000/orders/${props}`), {
      withCredentials: true,
    })
      .then(response => {
...
export default function Products() {
  ...
  const decodedCookie = parseJwt(access_token);
  let products = getOrders(Number(decodedCookie));
          ...
          <div>
            <section className="container mx-auto p-10 md:py-12 px-0 md:p-8 md:px-0">
              <div className="p-5 md:p-0 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-10 items-start">
                {products && Array.isArray(products) && products.map((r: any, i: number) => (
                  <div className="p-5 bg-white bg-opacity-90 text-center transform duration-500 hover:-translate-y-2 rounded-2xl text-black min-h-full" key={r.id}>
                    <h1 className="text-2xl my-5">Commandé le: {r.created_at}</h1>
                    ...
                    <h2 className="font-semibold mb-1 h-full">§ {r.price}</h2>
                    <p className="text-sm mb-5 align-middle grid items-center w-full m-0">Commander à nouveau:</p>
